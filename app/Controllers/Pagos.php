...
private function formatDates($pagos)
{
    ...
    public function enviarRecordatorioPago($correoDestino, $nombreInquilino, $monto)
    {
        $email = new Email();

        $email->setTo($correoDestino);
        $email->setFrom(config('Email')->fromEmail, config('Email')->fromName);
        $email->setSubject('Recordatorio de Pago');

        $mensaje = "Hola $nombreInquilino,<br><br>";
        $mensaje .= "Este es un recordatorio amistoso de que debes realizar un pago de $monto.<br><br>";
        $mensaje .= "Gracias,<br>Tu Arrendador";

        $email->setMessage($mensaje);

        if ($email->send()) {
            echo 'Correo electrónico de recordatorio de pago enviado correctamente';
        } else {
            echo 'Error al enviar el correo electrónico de recordatorio de pago';
        }
    }

    public function enviarConfirmacionPago($correoDestino, $nombreInquilino, $monto)
    {
        $email = new Email();

        $email->setTo($correoDestino);
        $email->setFrom(config('Email')->fromEmail, config('Email')->fromName);
        $email->setSubject('Confirmación de Pago');

        $mensaje = "Hola $nombreInquilino,<br><br>";
        $mensaje .= "Hemos recibido tu pago de $monto.<br><br>";
        $mensaje .= "Gracias,<br>Tu Arrendador";

        $email->setMessage($mensaje);

        if ($email->send()) {
            echo 'Correo electrónico de confirmación de pago enviado correctamente';
        } else {
